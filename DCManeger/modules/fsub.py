...
from DCManeger import BOT_ID
from DCManeger import DRAGONS as DEVS
from DCManeger import OWNER_ID
from DCManeger import telethn as dc
from DCManeger.events import dcinline
from DCManeger.events import register as dcbot
from DCManeger.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await dc(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await dc(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@dcbot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
