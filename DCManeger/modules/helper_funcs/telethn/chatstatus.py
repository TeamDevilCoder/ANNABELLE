...
from DCManeger import DRAGONS
from DCManeger.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def dc_is_admin(chat_id: int):
    status = False
    dc = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if dc.id == user.id:
